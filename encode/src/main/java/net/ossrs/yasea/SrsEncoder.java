...
public class SrsEncoder {
    ...
    public static int vOutWidth = 360;   // Note: the stride of resolution must be set as 16x for hard encoding with
    // some chip like MTK
    public static int vOutHeight = 640;  // Since Y component is quadruple size as U and V component, the stride must
    // be set as 32x
    ...
    public boolean start() {
        ...
        //videoFormat.setInteger("rotation-degrees",270);
        //videoFormat.setInteger(MediaFormat.KEY_QUALITY,5);
        ...
        vencoder.start();
        aencoder.start();
        Log.i(TAG, "start encoder done");
        return true;
    }

    public void pause() {
        mPauseTimeUs = System.nanoTime() / 1000;
    }

    public void resume() {
        mPresentTimeUs = mPresentTimeUs + (System.nanoTime() / 1000 - mPauseTimeUs);
    }
    ...
    public void setVideoHDMode() {
        vBitrate = 2048 * 1024*2;  // 1200 kbps
        x264Preset = "veryfast";
    }

    public void setVideoSmoothMode() {
        vBitrate = 1024 * 1024;  // 500 kbps
    ...
    public void onGetPcmFrame(byte[] data, int size) {
        ...
        // AtomicInteger videoFrameCacheNumber = mp4Muxer.getVideoFrameCacheNumber();
    ...
    public void onGetRgbaFrame(byte[] data, int width, int height, long timeStamp) {
        Log.i(TAG, "onGetRgbaFrame");
        // Check video frame cache number to judge the networking situation.
        // Just cache GOP / FPS seconds data according to latency.
        // AtomicInteger videoFrameCacheNumber = flvMuxer.getVideoFrameCacheNumber();
        // if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() < VGOP) {
        long pts = timeStamp / 1000 - mPresentTimeUs;
        if (useSoftEncoder) {
    ...
    private byte[] hwYUVNV21FrameScaled(byte[] data, int width, int height, Rect boundingBox) {
        switch (mVideoColorFormat) {
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar:
                return NV21ToI420Scaled(data, width, height, true, 180, boundingBox.left, boundingBox.top,
                        boundingBox.width(), boundingBox.height());
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar:
                return NV21ToNV12Scaled(data, width, height, true, 180, boundingBox.left, boundingBox.top,
                        boundingBox.width(), boundingBox.height());
            default:
                throw new IllegalStateException("Unsupported color format!");
        }
    ...
    private byte[] hwArgbFrameScaled(int[] data, int width, int height, Rect boundingBox) {
        switch (mVideoColorFormat) {
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar:
                return ARGBToI420Scaled(data, width, height, false, 0, boundingBox.left, boundingBox.top, boundingBox
                        .width(), boundingBox.height());
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar:
                return ARGBToNV12Scaled(data, width, height, false, 0, boundingBox.left, boundingBox.top, boundingBox
                        .width(), boundingBox.height());
            default:
                throw new IllegalStateException("Unsupported color format!");
        }
    ...
    private native byte[] ARGBToI420Scaled(int[] frame, int width, int height, boolean flip, int rotate, int crop_x,
                                           int crop_y, int crop_width, int crop_height);

    private native byte[] ARGBToNV12Scaled(int[] frame, int width, int height, boolean flip, int rotate, int crop_x,
                                           int crop_y, int crop_width, int crop_height);
    ...
    private native byte[] NV21ToNV12Scaled(byte[] frame, int width, int height, boolean flip, int rotate, int crop_x,
                                           int crop_y, int crop_width, int crop_height);

    private native byte[] NV21ToI420Scaled(byte[] frame, int width, int height, boolean flip, int rotate, int crop_x,
                                           int crop_y, int crop_width, int crop_height);
