...
import com.android.grafika.encoder.MediaVideoEncoder;
...
public class TextureMovieEncoder implements Runnable {
    ...
    public static class EncoderConfig {
        ...
        final float[] mTextureArray;
        final int mOrientation;

        public EncoderConfig(File outputFile, int width, int height, int bitRate,
                             EGLContext sharedEglContext, float[] textureArray, int orientation) {
    ...
    public void startRecording(EncoderConfig config) {
        Log.i(TAG, "Encoder: startRecording()");
    ...
    private void handleStartRecording(EncoderConfig config) {
        Log.i(TAG, "handleStartRecording " + config);
        mFrameNum = 0;
        prepareEncoder(config.mEglContext, config.mWidth, config.mHeight, config.mBitRate,
                config.mOutputFile, config.mTextureArray,config.mOrientation);
    }
    ...
    private void handleFrameAvailable(float[] transform, long timestampNanos) {
        if (VERBOSE) Log.i(TAG, "handleFrameAvailable tr=" + transform);
    ...
    private void handleUpdateSharedContext(EGLContext newSharedContext) {
        Log.i(TAG, "handleUpdatedSharedContext " + newSharedContext);
    ...
    private void prepareEncoder(EGLContext sharedContext, int width, int height, int bitRate,
                                File outputFile, float[] textureArray,int orientation) {
        Log.i(TAG, "prepareEncoder");
        try {
            mMuxer = new MediaMuxerWrapper(outputFile.getAbsolutePath(),orientation);
        ...
        mInputWindowSurface = new WindowSurface(mEglCore, mVideoEncoder.getSurface(), true);
        mInputWindowSurface.makeCurrent();

        mFullScreen = new FullFrameRect(
                new Texture2dProgram(Texture2dProgram.ProgramType.TEXTURE_EXT));
        mFullScreen.setRectTexture(textureArray);
        Log.i(TAG, "prepareEncoder");
    }
