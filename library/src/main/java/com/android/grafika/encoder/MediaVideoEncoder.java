...
public class MediaVideoEncoder extends MediaEncoder {
    ...
    public MediaVideoEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener, final int width,
                             final int height) {
   ...
   /* public void setEglContext(final EGLContext shared_context, final int tex_id) {
    ...
    private final int FRAME_BITRATE = 2048 * 1024 * 2;

    private int calcBitRate() {
        int bitrate = (int) (BPP * FRAME_RATE * mWidth * mHeight);
        Log.i(TAG, String.format("bitrate=%5.2f[Mbps]", bitrate / 1024f / 1024f));
        if (bitrate < FRAME_BITRATE)
